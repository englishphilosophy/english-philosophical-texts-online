...
import { ContentType, HttpError, headers } from "womble";
...
export const author = async (id: string): Promise<Response> => {
  ...
  return new Response(page.author(author, analysis).toString(), {
    status: Status.OK,
    headers: headers(ContentType.HTML),
  });
};
...
export const text = async (id: string): Promise<Response> => {
  ...
  return new Response(page.text(text, analysis).toString(), {
    status: Status.OK,
    headers: headers(ContentType.HTML),
  });
};
...
export const research = (
  id: "research" | "similarity" | "topics"
): Response => {
  return new Response(page.research(id).toString(), {
    status: Status.OK,
    headers: headers(ContentType.HTML),
  });
};
...
export const about = async (
  id: "about" | "corpus" | "principles" | "permissions" | "contact" | "support"
): Promise<Response> => {
  return id === "corpus"
    ? new Response(page.corpus(await read.authors()).toString(), {
        status: Status.OK,
        headers: headers(ContentType.HTML),
      })
    : new Response(page.about(id).toString(), {
        status: Status.OK,
        headers: headers(ContentType.HTML),
      });
};
...
export const javascript = async (path: string): Promise<Response> => {
  ...
  try {
    return new Response(await Deno.readTextFile(actualPath), {
      status: Status.OK,
      headers: headers(ContentType.JavaScript),
    });
  } catch {
...
export const error = (error: unknown): Response => {
  ...
  return new Response(page.error(httpError).toString(), {
