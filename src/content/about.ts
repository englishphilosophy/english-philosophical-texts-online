...
export const about = element('div', { class: 'content', children: [
...
export function corpus (authors: Author[]): Element {
  ...
  return element('div', { class: 'content', children: [
    ...
    element('p', { innerHTML: 'Our selection is based on the following general considerations:' }),
    element('ol', { children: [
      ...
      element('li', { innerHTML: 'The collection should include adequate female representation, with a minimum target of 20%. Happily we are exceeding that target comfortably at present.' })
    ] }),
    element('p', { innerHTML: 'We welcome suggestions for additional works of interest that we may have missed. Particularly welcome are works by neglected or forgotten female philosophers.' })
  ] })
}

/** Content for the editorial principles page. */
export const principles = element('div', { class: 'content', children: [
  ...
  element('p', { innerHTML: 'A silently edited critical text, without any accompanying critical apparatus, is of course of limited use for serious textual scholarship. But the editions provided on this site are not intended for that purpose.' }),
  ...
  element('p', { innerHTML: 'Some texts include small comments printed in the margin, typically indicating the topic currently being addressed in the main text. These comments are rendered here floating to the right of the paragraph, rather than in the actual margin.' }),
...
export const permissions = element('div', { class: 'content', children: [
...
export const contact = element('div', { class: 'content', children: [
  ...
  element('p', { innerHTML: 'For suggestions or queries, please contact Amyas Merivale at <a href="mailto:amyas.merivale@philosophy.ox.ac.uk">amyas.merivale@philosophy.ox.ac.uk</a>.' })
] })

/** Content for the support page (using statements of support from `statements.ts`). */
export const support = element('div', { class: 'content', children: [
  element('h1', { innerHTML: titles.support }),
  element('div', { children: statements.map(x => {
