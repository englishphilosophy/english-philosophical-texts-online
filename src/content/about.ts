import type { Author, TextStub } from "ept/library";
...
/** About page details. */
export const pages: PageDetails[] = [
  { id: "about", title: titles.about, url: "/about" },
  { id: "corpus", title: titles.corpus, url: "/about/corpus" },
  { id: "principles", title: titles.principles, url: "/about/principles" },
  { id: "permissions", title: titles.permissions, url: "/about/permissions" },
  { id: "contact", title: titles.contact, url: "/about/contact" },
  { id: "support", title: titles.support, url: "/about/support" },
];

/** Content for the main about page. */
export const about = element("div", {
  class: "content",
  children: [
    element("h1", { innerHTML: titles.about }),
    element("p", {
      innerHTML:
...
export const corpus = (authors: Author[]): Element => {
  ...
  const texts = authors.reduce(
    (sofar: TextStub[], current) => sofar.concat(current.texts),
    []
  );
  ...
  const authorsWithTextsList = authorsWithTexts.reduce(
    (sofar, current, index) => {
      ...
      if (index === authorsWithTexts.length - 1)
        return `${sofar}, and ${misc.link(current)}`;
  ...
  return element("div", {
    class: "content",
    children: [
      element("h1", { innerHTML: titles.corpus }),
      element("p", {
        innerHTML: `The corpus on this site contains ${
          texts.length - duplicatedTexts.length / 2
        } English-language philosophical texts published in Britain between 1650 and 1830, by ${
          authors.length
        } authors (${maleAuthors.length} male and ${
          femaleAuthors.length
        } female). Our aim is to provide free and high-quality digital critical texts for all of the works in our corpus, alongside tools for performing sophisticated searches and comparative textual analyses. Note however that this project is still in the early stages. So far we have prepared ${
          importedTexts.length
        } texts by ${authorsWithTextsList}.`,
      }),
      element("p", {
        innerHTML:
          "Our selection is based on the following general considerations:",
      }),
      element("ol", {
        children: [
          element("li", {
            innerHTML:
              "Works should be originally published in English. This restriction is necessary because of the kinds of computational linguistic analyses we want to run, which only make sense applied to a single language. Hobbes and Conway necessitate bending this rule slightly. We have included the English versions of Hobbes’s works that he himself published (even if he published a Latin version first), but excluded his Latin works that were only translated posthumously. The original English manuscript of Conway’s <em>Principles of Ancient and Modern Philosophy</em>, meanwhile, does not survive. It was first published in a Latin translation, then posthumously translated from the Latin back into English; we have included this third-hand English text, for want of anything better.",
          }),
          element("li", {
            innerHTML:
              "Works should be philosophical in a suitably broad sense, given the period, and consequently the list includes many works on what we would now consider more as theology or social science. We have even included two histories of England (Hume’s and Macaulay’s), but only because of the independent philosophical interest in these writers; we are not including histories more generally.",
          }),
          element("li", {
            innerHTML:
              "Works should be written by a known British author (the highly influential Dutch-born immigrant Mandeville being a partial exception to this rule). This is simply to keep things within reasonable bounds. Works by unknown authors or North American writers could be added in the future. (This is why—with apologies to the Irish, Scottish, and Welsh—we have called this site <em>English</em> Philosophical Texts Online, and not <em>British</em> Philosophical Texts Online. As with the similarly titled <em>Early English Books Online</em>, it is to the language and not the nation that we are referring.)",
          }),
          element("li", {
            innerHTML:
              "The collection should include adequate female representation, with a minimum target of 20%. Happily we are exceeding that target comfortably at present.",
          }),
        ],
      }),
      element("p", {
...
export const principles = element("div", {
  ...
  children: [
    ...
    element("h2", { innerHTML: "Formatting" }),
    element("p", {
      innerHTML:
    ...
    element("h2", { innerHTML: "Margin Comments" }),
    element("p", {
      innerHTML:
    ...
    element("h2", { innerHTML: "Apostrophes" }),
    element("p", {
      innerHTML:
...
export const contact = element("div", {
  class: "content",
  children: [
    element("h1", { innerHTML: titles.contact }),
    element("p", {
      innerHTML:
    ...
    element("p", {
      innerHTML:
        'For suggestions or queries, please contact Amyas Merivale at <a href="mailto:amyas.merivale@philosophy.ox.ac.uk">amyas.merivale@philosophy.ox.ac.uk</a>.',
    }),
  ],
});

/** Content for the support page (using statements of support from `statements.ts`). */
export const support = element("div", {
  class: "content",
  children: [
    element("h1", { innerHTML: titles.support }),
    element("div", {
      children: statements.map((x) => {
        const innerHTML = x.affiliation
          ? `${x.from}, ${x.affiliation}`
          : x.from;
        return element("div", {
          children: [
            element("blockquote", { innerHTML: x.text }),
            element("cite", { innerHTML }),
          ],
        });
      }),
    }),
