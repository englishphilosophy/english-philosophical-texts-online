import type { Analysis, Lemma } from "ept/analysis";
  ...
  return element("div", {
    ...
    children: [
      ...
      element("ul", {
        children: [
          element("li", {
            innerHTML: `${analysis.wordCount} words and ${analysis.lemmaWordCount} lexemes`,
          }),
          element("li", {
            innerHTML: `${analysis.names.length} references to named people (totalling ${analysis.nameWordCount} words)`,
          }),
          element("li", {
            innerHTML: `${analysis.citations.length} citations (totalling ${analysis.citationWordCount} words)`,
          }),
          element("li", {
            innerHTML: `${analysis.foreignText.length} instances of foreign text (totalling ${analysis.foreignWordCount} words)`,
          }),
        ],
      }),
    ],
...
const warning = element("p", {
  class: "warning",
  innerHTML:
    "These data are provisional. Their accuracy depends on software that is still being developed, and manual markup that is still being inputted and checked.",
});
