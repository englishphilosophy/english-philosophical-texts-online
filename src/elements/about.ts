...
export const author = (author: Author): Element => {
  return element("div", {
    ...
    children: [
      element("h2", {
        innerHTML: `${misc.fullname(author)} (${author.birth}-${author.death})`,
      }),
...
export const text = (text: Text): Element => {
  const children = [
    ...
    element("p", {
      innerHTML: text.published.map((x) => x.toString(10)).join(", "),
    }),
    element("h4", { innerHTML: "Copytext" }),
    element("p", {
      innerHTML: text.copytext.map((x) => x.toString(10)).join(", "),
    }),
  ];
  if (text.sourceUrl) {
    ...
    children.push(
      element("p", {
        children: [
          element("a", { href: text.sourceUrl, innerHTML: text.sourceUrl }),
        ],
      })
    );
  }
